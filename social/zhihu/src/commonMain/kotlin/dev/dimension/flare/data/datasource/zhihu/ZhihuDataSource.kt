...
internal class ZhihuDataSource(
    ...
    private val loader by lazy { ZhihuLoader(accountKey = accountKey, service = service) }

    private val notificationBadgeStore = ZhihuNotificationBadgeStore(loader) {}
    ...
    override val supportedNotificationFilter: List<NotificationFilter> = listOf(
    ...
    override val builtInTimelineTabs: ImmutableList<TimelineCandidate<*>> by lazy {
        persistentListOf(
            ...
            ZhihuPlatformSpec.momentsTimelineSpec.candidate(
                data = TimelineSpec.AccountBasedData(accountKey),
                icon = IconType.Material(UiIcon.Follow),
            ),
            ZhihuPlatformSpec.bookmarkTimelineSpec.candidate(
                data = TimelineSpec.AccountBasedData(accountKey),
                icon = IconType.Material(UiIcon.Bookmark),
            ),
            ZhihuPlatformSpec.likeTimelineSpec.candidate(
                data = TimelineSpec.AccountBasedData(accountKey),
    ...
    override fun notification(type: NotificationFilter): RemoteLoader<UiTimelineV2> = when (type) {
        NotificationFilter.All -> ZhihuNotificationTimelineLoader(service = service, accountKey = accountKey)
        NotificationFilter.Mention -> ZhihuNotificationTimelineLoader(
            service = service, accountKey = accountKey, filter = NotificationFilter.Mention,
        )
        NotificationFilter.Comment -> ZhihuNotificationTimelineLoader(
            service = service, accountKey = accountKey, filter = NotificationFilter.Comment,
        )
        NotificationFilter.Like -> ZhihuNotificationTimelineLoader(
            service = service, accountKey = accountKey, filter = NotificationFilter.Like,
        )
    }

    /**
     * 检查登录状态，过期则抛 LoginExpiredException
     */
    ...
    override fun composeConfig(type: ComposeType): ComposeConfig =
        ComposeConfig(
            ...
            media = ComposeConfig.Media(9, false, altTextMaxLength = -1, allowMediaOnly = false),
        )
    ...
    fun momentsTimelineLoader(): RemoteLoader<UiTimelineV2> = ZhihuMomentsTimelineLoader(service = service, accountKey = accountKey)
    fun bookmarkTimelineLoader(): RemoteLoader<UiTimelineV2> = ZhihuBookmarkTimelineLoader(service = service, accountKey = accountKey)
    fun likeTimelineLoader(): RemoteLoader<UiTimelineV2> = ZhihuLikeTimelineLoader(service = service, accountKey = accountKey)

    /**
     * 楼中楼评论
     */
    fun commentChild(statusKey: MicroBlogKey, parentCommentId: String): RemoteLoader<UiTimelineV2> =
        ZhihuCommentChildLoader(service = service, accountKey = accountKey, statusKey = statusKey, parentCommentId = parentCommentId)
}
